...
import { corsHeaders, CACHE_CONFIG, getSefariaUrl } from "./config.ts";
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const now = new Date();
    const maxAgeDate = new Date(now.getTime() - CACHE_CONFIG.maxAgeDays * 24 * 60 * 60 * 1000);
    const staleDate = new Date(now.getTime() - CACHE_CONFIG.staleAfterDays * 24 * 60 * 60 * 1000);

    const { error: deleteError } = await supabase
      .from("sefaria_cache")
      .delete()
      .lt("cached_at", maxAgeDate.toISOString())
    ...
    const sefariaUrl = getSefariaUrl(normalizedRef);
